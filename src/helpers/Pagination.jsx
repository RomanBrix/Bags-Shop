...
export default function Pagination({ itemsPerPage, items, urlItemOffset = 0, setCurrentItems, filters}) {
  const [clickedTarget, setClickedTarget] = useState(urlItemOffset + 1)
  const navigate = useNavigate()
  const [pageCount, setPageCount] = useState(0);
  const [itemOffset, setItemOffset] = useState(urlItemOffset * itemsPerPage % items.length);
  ...
  // eslint-disable-next-line
  useEffect(()=>{ navigate(`?page=${clickedTarget}`)},[clickedTarget])
  useEffect(()=>{
